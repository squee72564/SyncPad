...
- Document share links now have a dedicated router under `/v1/workspaces/:workspaceId/documents/:documentId/share-links` (list/create/update/delete) plus `/v1/share-links/:token` for public previews. Use these when exposing read/comment/edit tokens to non-members; they rely on `share:create` / `share:manage` permissions and reuse the middleware’s `allowShareLinks` option for token-based access.
