...
- Activity logs are exposed via `/v1/workspaces/:workspaceId/activity-logs` (POST to append entries, DELETE to prune) and require `workspace:manage`; each log may reference an actor, document, and arbitrary metadata, giving downstream services a single workspace timeline.
