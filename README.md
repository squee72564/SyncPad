...
- **Activity Logs**: Workspace timelines can now be written via `/v1/workspaces/:workspaceId/activity-logs` (POST to create, DELETE to remove) so backend services capture document/member events with optional actor/document metadata.
- **Dashboard UX**: Next.js dashboard now includes a workspace switcher, API-backed document listings (all, drafts, published), invite management (composer, pending list, copy/resend/revoke actions), document share-link management (document picker, creator form, pending list with copy/regenerate/revoke), and forms for creating/updating document metadata. Public invite/share-link acceptance lives under `/invites/:token` and `/share-links/:token` with auth-aware redirects.
