...
- When the API runs outside the Docker network, set `BACKEND_REDIS_URL` so `backend/src/lib/redis.ts` can reach the Redis host while the embedding worker continues using `REDIS_URL`.
- `document.service.ts` compares old/new statuses during create/update/delete. When a document enters a RAG-eligible state (`PUBLISHED` or `ARCHIVED`), it materializes the latest collab snapshot into `Document.content`, creates a `DocumentRevision`, and enqueues an `EMBEDDING` job with that `revisionId`. When it leaves those states or is deleted, `document-embedding.service.ts` removes existing vectors.
...
- **AI Job Strategy**: The `AiJob` table plus worker pipeline should progress through three tiers:
  - *Tier 1 (foundations)*: `EMBEDDING` jobs chunk documents and upsert vectors; `SUMMARIZE_DOCUMENT`/`WORKSPACE_DIGEST` jobs produce summaries/digests; `DRAFT_OUTLINE` assists the editor with outline suggestions surfaced via server actions.
  - *Tier 2 (collaborative coauthors)*: `COLLAB_EDIT` jobs create tracked-change suggestions; `INSIGHT_GENERATOR` highlights risks based on document + activity context; `COMMENT_SUMMARY`/`RESOLUTION_PROPOSAL` keep long threads manageable.
  - *Tier 3 (autonomous memory)*: `COAUTHOR_ROOM` multi-agent workshops coordinate role-specific agents, `DECISION_TIMELINE` jobs build a decision graph from activity logs, and `IDEATION_SPRINT` jobs aggregate docs to propose plans/tasks. Always respect workspace RBAC/share-link scopes when invoking models, and persist results so activity logs remain the system of record.
...
- Draft documents now render via TipTap (MIT core) in `/dashboard/documents/[id]`, storing structured JSON snapshots in `DocumentCollabState` via `saveDocumentCollabStateAction`. Publish/archive now materializes the latest snapshot into `DocumentRevision` + `Document.content` before embedding. Plan to swap the snapshot storage for a Yjs/ProseMirror CRDT state when the realtime service is ready.
