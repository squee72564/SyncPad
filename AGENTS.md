...
- `document.service.ts` compares old/new statuses during create/update/delete. When a document enters a RAG-eligible state (`PUBLISHED` or `ARCHIVED`), it enqueues an `EMBEDDING` job. When it leaves those states or is deleted, `document-embedding.service.ts` removes existing vectors.
