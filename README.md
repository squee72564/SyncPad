...
- **Embedding Worker**: The `embedding_worker/` service consumes Redis stream jobs, chunks document content, generates embeddings through a configurable provider (OpenAI/self-hosted), and stores vectors in PostgreSQL. The Express API now enqueues jobs whenever a document enters/exits `PUBLISHED`/`ARCHIVED` status, materializing the latest collab snapshot into a `DocumentRevision`/`Document.content` and passing the `revisionId` so embeddings are tied to the captured content; vectors are deleted when it becomes draft again.
