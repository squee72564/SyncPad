...
export default async function DocumentPage({
    ...
    return (
      <div className="flex flex-col gap-4 p-6">
        ...
        <div className="rounded-lg border border-dashed border-muted-foreground/40 p-6 text-sm text-muted-foreground">
          <p className="mb-4">
            You do not  an active workspace. Choose one from the sidebar or create a new
            workspace to begin organizhaveing documents.
          </p>
  ...
  const document = await getDocument(activeWorkspace.workspace.id, activeWorkspace.workspace.slug, id);
